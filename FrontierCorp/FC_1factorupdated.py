...
def calibration_func( sig_A, E_market, sig_E, N, r, M, A ):

    #Aux functions if needed
#    d1 = (math.log(A/N) + ((r + 0.5*(CalibSigA**2))*M))/(CalibSigA*math.sqrt(M))
#    d2 = (math.log(A/N) + ((r - 0.5*(CalibSigA**2))*M))/(CalibSigA*math.sqrt(M)) 
    
    ## 1-factor: no d1/d2
    out = ((sig_E*E_market/float(sig_A)) - (N*math.exp(-1.0*r*M)*norm.cdf( norm.ppf(sig_E*E_market/float(A*sig_A)) - sig_A*math.sqrt(M) )) - E_market)
    
    ## First Equation
    #out = A*norm.cdf(d1) - N*math.exp(-1*(r*M))*norm.cdf(d2) - E_market
  
    ## Second Equation
    #out = sig_A*A*norm.cdf(d1) - sig_E*E_market
             
    return out

def delta_spread( A, CalibSigA, E_market, sigma_E_market, N, r, M, spd ):
    
    d1 = (math.log(A/N) + ((r + 0.5*(CalibSigA**2))*M))/(CalibSigA*math.sqrt(M))
    d2 = (math.log(A/N) + ((r - 0.5*(CalibSigA**2))*M))/(CalibSigA*math.sqrt(M))    
    
    calib_spread = (-1.0/M)*math.log( norm.cdf(d2) + ((A*math.exp(r*M))/N)*norm.cdf(-1.0*d1) )
    calib_spread *= 10000.0
    
    return (spd-calib_spread)


#establish the map between quarter and paramteres for FC
...
#firstEq = open('FC_firstEq.csv', 'w')
#secondEq = open('FC_secondEq.csv', 'w')
subEq = open('FC_subEq.csv', 'w')
for k in dateToData.keys():
    ...
    if (spd != 0.0):
        ...
        try:
            ...
            ## Calibrate
            CalibSigA = float(fsolve( calibration_func, sig_E, args=(E, sig_E, N, r, M, A) ))
            ...
            d_spread = delta_spread( A, CalibSigA, E, sig_E, N, r, M, spd )

            ## Get LGD / Beta_BM (Beta_EM*constants)
            LGD = 1.0 - ( ( math.exp(r*M) * A * norm.cdf(-d1) ) / ( N*norm.cdf(-d2)  ) )
            PD = norm.cdf(-d1)
            bta_BM = ( (norm.cdf(-d1)*E*bta) / (float(B*norm.cdf(d1))) )

            ## Output, all files
            out_str1 = str(bta_BM) + "," + str(CalibSigA) + "," + str(bta) + "," + str(d_spread) + "\n"
            #firstEq.write( out_str1 )
            #secondEq.write( out_str1 )
            
            
            # remove the 10^-127 values
            if ( bta_BM < 0.0000001 ):
