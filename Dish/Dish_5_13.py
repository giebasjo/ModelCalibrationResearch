...
"""
The function below is the BSM. We will
use this to compare the historical
data and calibrated data as inputs to
the BSM model, then compare the Betas
from the BSM model to our Model.

Equity is a call option on Assets.

"""
def BSM(A, N, sigma_A, r, M):
    
    d1 = (math.log(A/N) + (r + 0.5*(sigma_A**2))*M)/(sigma_A*math.sqrt(M))
    d2 = (math.log(A/N) + (r - 0.5*(sigma_A**2))*M)/(sigma_A*math.sqrt(M))
    
    return A*norm.cdf(d1) - N*math.exp(-1.0*r*M)*norm.cdf(d2)
...
def fsolve_function( init, E_market, sigma_E_market, r, M, N ):
...
book = xlrd.open_workbook( "Dish_1factor.xlsx" )
...
M = 5.0 # assume
outFile  = open('Dish_5_12.csv','w')
outFile2 = open('Dish_staticOutputFile5_13.csv','w')
outFile2.write( "Date,SigE,CalibSigA,CalibN,Equity,Beta_BM,Beta_EM,DeltaSpread,PD,LGD,\n" )
...
for k in dateToData.keys():
    ...
    # choose Notional to be Liabilities per Dr. Murphy's insight
    N = B
    
    # get an initial guess for Assets through Miller-Modigliani
    A_init = E + B
    
    initial_guess = [0.2, A_init]
    ...
    try:
        ...
        CalibSigA, CalibA = fsolve( fsolve_function, initial_guess, args=(E, sig_E, r, M, N) )
        
        CalibSigA_List.append( CalibSigA )
        ...
        # d1/d2 aux functions
        d1 = (math.log(CalibA/N) + ((r + 0.5*(CalibSigA**2))*M))/(CalibSigA*math.sqrt(M))
        d2 = (math.log(CalibA/N) + ((r - 0.5*(CalibSigA**2))*M))/(CalibSigA*math.sqrt(M))    
        ...
        d_spread, calib_spread = delta_spread( CalibA, CalibSigA, E, sig_E, N, r, M, spd )
        d_spreadList.append( d_spread )
        
        """
        5/15/2017
        
        Identify date, and other params, where
        the DeltaSpread is highly negative (i.e.
        when the CalibSpread > PhySpread)
        
        I choose points to identifiy if the delta
        spread is < -1000, or if the calibrated spread
        is 1000 BP greater than the physical spread.
        """
        ...
        if d_spread < -1000:
            ...
            print("   Liabilities: ", B)
            print("   Equity: ", E)
            print("   Spread_phys: ", spd)
            print("   STD: ", STD)
            print("   LTD: ", LTD)
            print("\nCalibrated Params Below: \n")
            print("   CalibA: ", CalibA)
            print("   CalibSigA: ", CalibSigA)
            print("   CalibSpread: ", calib_spread)
            print("   DeltaSpread: ", d_spread)

        """        

        ## Loss Given Default (LGD), Probability of Default (PD)
        LGD = 1.0 - ( ( math.exp(r*M) * CalibA * norm.cdf(-d1) ) / ( N*norm.cdf(-d2)  ) )
        PD = norm.cdf(-d1)
        
        LGD_List.append( LGD )
        PD_List.append( PD )
        
        outStr = str(k) + "," + str(beta_BM) + "," + str(d_spread) + "," + \
                    str(calib_spread) + "," + str(spd) + "," + str(LGD) + "," + str(PD) + "\n"
                    
        outFile.write( outStr )
        
        outStr2 = str(k) + "," + str(sig_E) + "," + str(CalibSigA) + "," + str(N) + "," + str(E) + "," + \
                     str(beta_BM) + "," + str(bta) + "," + str(d_spread) + "," + str(PD) + "," + str(LGD) + "\n"
...
plt.xlabel("Beta_{B,M}")
plt.ylabel("DeltaSpread")
plt.plot(betaBM_List, d_spreadList, 'bo')
plt.show()

print("\navg Beta_BM: ", averageOfList( betaBM_List ) )
