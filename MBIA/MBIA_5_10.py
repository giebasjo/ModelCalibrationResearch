...
"""
SOME OF THE RATES ARE -1!!!!!! THIS CANNOT HAPPEN,
WE MUST SKIP THESE DATES!
"""
...
## Put in the MktCap (Equity) data
dates_Mkt = Mkt_data.col_slice( colx=0, start_rowx=0 )
nEntries_Mkt = len( dates_Mkt )
for i in range( 0, nEntries_Mkt ):
    
    date_i_float_Mkt = Mkt_data.cell_value(i,0)
    date_i_formatted_Mkt = datetime.datetime( *xlrd.xldate_as_tuple( date_i_float_Mkt, book.datemode ) ).strftime('%d-%m-%Y')    
    dateToData[ date_i_formatted_Mkt ].append( float( Mkt_data.cell_value( i,1 ) ) )
