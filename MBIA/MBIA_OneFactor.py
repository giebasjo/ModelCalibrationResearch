...
val_error = 0
#outFile_new = open('updated_3-30.csv', 'w')
#outFile_new2 = open('updated_3-30_justBetaEM.csv', 'w')
#otherFile = open('spreads_exact.csv', 'w')
mbia_outFile = open('MBIA_results2.csv', 'w')
mbia_PdLgdFile = open('mbia_PdLgdResults.csv', 'w')
mbia_sigAFile = open('testSigAMBIA.csv','w')
...
for k in dateToData.keys():
    ...
    if (spd != 0.0):
        ...
        try:
            ...
            mbia_outFile.write( out_str )
            
            out_str2 = str(bta) + "," + str(LGD) + "," + str(PD) + "\n"
            mbia_PdLgdFile.write( out_str2 )
            
            out_str3 = str(CalibSigA) + "\n"
            mbia_sigAFile.write( out_str3 )
